<div>
    ...
    <div class="row mb-3">
        <div class="col-12">
            @if($session_date && $session_tgl_bongkar && $session_supplier && $session_jenis_penerimaan || $session_date || $session_tgl_bongkar || $session_supplier || $session_jenis_penerimaan)
                @php
                    $selectedSupplier = $suppliers->firstWhere('supplier_id', $session_supplier);
                @endphp
            ...
            @else
                <div class="alert alert-info">
                    ...
                    Pilih tanggal penerimaan, tanggal bongkar, supplier, dan jenis penerimaan untuk melihat data
                </div>
            @endif
        </div>
    </div>
                    ...
                    <tr>
                        ...
                        <td class="action-column">
                            ...
                            <button type="button" class="btn btn-outline-danger action-btn" data-bs-toggle="modal"
                                data-bs-target="#deleteModal{{ $item->penerimaan_id }}" title="Hapus {{ $item->supplier->nama_supplier }} - {{ $item->grade->grade }} ({{ $item->berat_ikan }}kg)">
    ...
    @foreach ($data as $item)
            ...
            <div wire:ignore.self class="modal fade" id="editModal{{ $item->penerimaan_id }}" tabindex="-1"
                ...
                <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="editModalLabel{{ $item->penerimaan_id }}">Edit Penerimaan Ikan - {{ $item->grade->grade }} ({{ $item->berat_ikan }}kg)
                            </h5>
                            <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                                <i data-feather="x"></i>
                            </button>
            ...
            <div class="modal fade" id="deleteModal{{ $item->penerimaan_id }}" tabindex="-1" role="dialog"
                ...
                <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="deleteModalLabel{{ $item->penerimaan_id }}">Hapus Penerimaan Ikan - {{ $item->grade->grade }} ({{ $item->berat_ikan }}kg)</h5>
                            <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                                <i data-feather="x"></i>
                            </button>
                        </div>
