...
@section('content')
        ...
        <div class="page-heading">
            ...
            <section class="section">
                                    ...
                                    <div class="modal-body">
                                        <form method="POST" action="{{ route('penerimaan_ikan.store') }}"
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select name="ikan_id" id="ikan_id" class="form-control border-primary"
                                                    required>
                ...
                <div class="card-body">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-outline-primary block"
                                                ...
                                                data-bs-target="#editPenerimaanModal{{ $item->id }}">
                                                Edit Penerimaan
                                            </button>
                                            <button type="button" class="btn btn-outline-danger block"
                                                data-bs-toggle="modal"
                                                data-bs-target="#hapusPenerimaanModal{{ $item->id }}">
                                                Hapus Penerimaan
                                            </button>
                                        </td>
                                    </tr>
                                    <!-- Modal Edit Penerimaan -->
                                    <div class="modal fade" id="editPenerimaanModal{{ $item->id }}" tabindex="-1"
                                        role="dialog" aria-labelledby="editPenerimaanModalTitle{{ $item->id }}"
                                        aria-hidden="true">
                                            ...
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title"
                                                        id="editPenerimaanModalTitle{{ $item->id }}">Edit Penerimaan
                                                    </h5>
                                                ...
                                                <div class="modal-body">
                                                    <form method="POST"
                                                        action="{{ route('penerimaan_ikan.update', $item->id) }}"
                                                        enctype="multipart/form-data" class="mt-0">
                                                        @csrf
                                                        @method('PUT')
                                                        <div class="form-group">
                                                        ...
                                                        <div class="form-group">
                                                            <label for="ikan_id">Ikan</label>
                                                            <select name="ikan_id" class="form-control border-primary"
                                                                required>
                                                                @foreach ($ikans as $ikan)
                                                                    <option value="{{ $ikan->id }}"
                                                                        {{ $ikan->id == $item->ikan_id ? 'selected' : '' }}>
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <input type="date" name="tgl_penerimaan"
                                                                ...
                                                                value="{{ $item->tgl_penerimaan }}" required>
                                                        </div>
                                                        <div class="form-group"></div>
                                                        <label for="berat_ikan">Berat Ikan</label>
                                                        <input type="number" name="berat_ikan" step="0.01"
                                                            class="form-control border-primary"
                    ...
                    <div class="modal fade" id="hapusPenerimaanModal{{ $item->id }}" tabindex="-1" role="dialog"
                        aria-labelledby="hapusPenerimaanModalTitle{{ $item->id }}" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="hapusPenerimaanModalTitle{{ $item->id }}">Hapus
                                        Penerimaan</h5>
                                    <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                                        <i data-feather="x"></i>
                                    </button>
                                ...
                                <div class="modal-footer">
                                    ...
                                    <form method="POST" action="{{ route('penerimaan_ikan.destroy', $item->id) }}"
                                        class="d-inline">
                                        @csrf
                                        @method('DELETE')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const beratIkanInput = document.querySelector('input[name="berat_ikan"]');
